...
module.exports = grammar({
  ...
  externals: ($) => [$.command_stop, $.paragraph_stop, $._eol, $.preamble_stop],
  ...
  rules: {
    ...
    preamble: ($) =>
      prec.left(seq(repeat($.preamble_content), $.preamble_stop)),

    main: ($) =>
      prec.right(18, seq($.main_start, repeat1($.paragraph), $.main_stop)),
    ...
    paragraph: ($) =>
      prec.right(14, seq(repeat1($._paragraph_content), $.paragraph_stop)),
    ...
    brace_group: ($) =>
      prec(
        10,
        seq($.brace_group_start, repeat($._group_content), $.brace_group_stop)
      ),
    ...
    _math_content: ($) =>
      choice($.comment, $.escaped, $.math_group, $.math_text),
    ...
    metapost_inclusion: ($) =>
      prec(10, seq($.metapost_start, $.metapost_body, $.metapost_stop)),
    ...
    tikz_inclusion: ($) =>
      prec(10, seq($.tikz_start, $.tikz_body, $.tikz_stop)),
    ...
    typing_html_inclusion: ($) =>
      prec(
        10,
        seq($.typing_html_start, $.typing_html_body, $.typing_html_stop)
      ),
      ...
      prec(
        ...
        seq(
          ...
          optional(
            seq(
              $.keyword,
              repeat(seq(",", optional($._end_of_line), $.keyword))
            )
          ),
          optional(","),
    ...
    title_setting: ($) =>
      prec.right(seq("title", "=", optional($.title_value))),
    ...
    command: ($) =>
      prec.right(
        ...
        seq(
          $.command_name,
          repeat(
            choice($.option_block, $.settings_block, $.comment, $._end_of_line)
          ),
          $.command_stop
        )
      ),
